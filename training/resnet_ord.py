...
SAVE_PATH = f"resnet_img_ord.pth"
...
class KneeDataset(Dataset):
    def __init__(self, df, transform=None):
        ...
        # CHANGED: store labels once
        self.labels = torch.tensor(
            [class_to_idx[v] for v in self.df["label"].values],
            dtype=torch.long
        )

        # CHANGED: ordinal targets
    ...
    def __getitem__(self, idx):
        ...
        # CHANGED: return both
        return img, self.labels[idx], self.labels_ord[idx]
...
# CHANGED: output = num_ord
model.fc = nn.Sequential(
    nn.Dropout(0.3),
    nn.Linear(model.fc.in_features, num_ord)
)
...
# =====================
# CHANGED: ordinal loss
# =====================
pos_weight = torch.ones(num_ord).to(device)
thresholds = torch.linspace(0.45,0.60,num_ord).to(device)
...
def ordinal_loss(logits, targets):
    ...
    for i in range(1, num_ord):
        mono += torch.relu(logits[:,i] - logits[:,i-1]).mean()

    return bce + 0.05 * mono

optimizer = optim.AdamW([
    {"params": [p for n,p in model.named_parameters() if "fc" not in n], "lr":3e-5},
...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        for x, yb, yb_ord in val_loader:
            ...
            # CHANGED
            pred_batch = (probs > thresholds.cpu().numpy()).sum(axis=1)

            preds.extend(pred_batch)
            all_probs.extend(probs)
            labels.extend(yb.cpu().numpy())

            correct += (pred_batch == yb.cpu().numpy()).sum()
            total += yb.size(0)
    ...
    print(f"Epoch {epoch+1} | Train Loss {train_loss:.4f} Acc {train_acc:.4f} | Val Loss {val_loss:.4f} Acc {val_acc:.4f} | F1 {f1:.4f} BalAcc {bal:.4f}")
...
np.save("resnet_probs_ord.npy", np.array(test_probs))
pd.DataFrame({"true": test_labels, "pred": test_preds}).to_csv("resnet_preds_ord.csv", index=False)
# =====================
# TEST ROC Curve
# =====================
...
plt.ylabel("TPR")
plt.legend()
plt.savefig("test_roc_curve_resnet_ord.png")
plt.close()
...
plt.savefig("test_distribution_resnet_ord.png")
plt.close()
